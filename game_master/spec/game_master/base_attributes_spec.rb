...
describe GameMaster::BaseAttributes do
  let(:test_class) do
    Class.new do
      include GameMaster::Base
      define_attribute :test_this, type: String, default: ""
    end
  end
  ...
  describe ".define_attribute_class" do
    it "should set a attribute_class to new value" do
      test_class.define_attribute_class(String)
      test_class.attribute_class.should eq String
    end
    it "should log an info message" do
      test_class #makes sure we're not catching log stuff for class creation
      Yell['GameMaster'].should_receive(:info).with(/Defining new Attribute Class/)
      test_class.define_attribute_class(String)
    end
  end
  ...
  describe "#define_attribute" do
    describe "should create methods" do
      ...
      it{test_class.instance_methods(false).should include(:test_this_default)}
      it{test_class.instance_methods(false).should include(:test_this_default=)}
    end
